...
{
    public class NameCollectHashRangeVirtualTableRoute : AbstractShardingOperatorVirtualTableRoute<NameCollect, Guid>
    {

        private const string Tables = "Tables";
        private const string TABLE_NAME = "TABLE_NAME";

        private const string CurrentTableName = middleSafeServerConsts.DbTablePrefix + "NameCollects";//这个是你数据库的表名吗
        ...
        public NameCollectHashRangeVirtualTableRoute(IVirtualDataSource virtualDataSource, IShardingTableCreator shardingTableCreator)
        {
                ...
                using (var dataTable = connection.GetSchema(Tables))
                {
                    for (int i = 0; i < dataTable.Rows.Count; i++)
                    {
                            ...
                            if (tableName.StartsWith(CurrentTableName +"_", StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                _tails.TryAdd(tableName.Substring(CurrentTableName.Length + 1), null);//这个不对
                            }
        ...
        public override void Configure(EntityMetadataTableBuilder<NameCollect> builder)
        {
            ...
            if (!_tails.TryGetValue(shardingKeyToTail, out var _))
            {
                lock (_lock)
                {
                    if (!_tails.TryGetValue(shardingKeyToTail, out var _))
                    {
                        try
                        {
                            _shardingTableCreator.CreateTable<NameCollect>(_virtualDataSource.DefaultDataSourceName,
                                shardingKeyToTail);
                        }
                        catch (Exception ex)
                        {
