...
bool CJsonObject::Get(const std::string& strKey, float& fValue) const
{
	...
	if (pJsonStruct->type == cJSON_String && NULL != pJsonStruct->valuestring)
	{
		fValue = strtof(pJsonStruct->valuestring, NULL);
		return(true);
	}
...
bool CJsonObject::Get(const std::string& strKey, double& dValue) const
{
	...
	if (pJsonStruct->type == cJSON_String && NULL != pJsonStruct->valuestring)
	{
		dValue = strtod(pJsonStruct->valuestring, NULL);
		return(true);
	}
    return(false);
}
...
{
	char _FString[100] = { NULL };
	sprintf_s(_FString, 100, "%f", fValue);
	return Add(strKey, _FString);
}

bool CJsonObject::Add(const std::string& strKey, double dValue)
{
	char _FString[100] = { NULL };
	sprintf_s(_FString, 100, "%lf", dValue);
	return Add(strKey, _FString);
}
